...
function unlockAudioOnce() {
  ...
  // Start background music on first interaction (browsers require a user gesture)
...
// ------------------------------------------------------------
// Screen shake state (VIEW — triggered by PlayerController via window helper)
// ------------------------------------------------------------
let shakeFrames = 0;
let shakeAmt = 0;

window.triggerScreenShake = function (amt = 3, frames = 5) {
  shakeAmt = amt;
  shakeFrames = frames;
};
...
async function boot() {
  ...
  // Load all available SFX (paths match assets/sfx/ folder)
  ...
  // No dedicated die/win files yet — reuse closest match; swap when you add them
  soundManager.load("die", "assets/sfx/receiveDamage.wav");
  soundManager.load("win", "assets/sfx/leafCollect.wav");

  // Background music (looping)
  soundManager.loadLoop("music", "assets/sfx/music.wav");
...
function draw() {
  ...
  // Screen shake: nudge canvas randomly for shakeFrames frames.
  if (shakeFrames > 0) {
    camera.off();
