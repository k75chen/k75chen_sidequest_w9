...
export class PlayerController {
  // debugOverlay is optional — passed in by Game so god mode flag is readable.
  constructor(playerEntity, opts = {}) {
    this.player = playerEntity;
    this.events = opts.events || null;
    this.debugOverlay = opts.debugOverlay || null; // NEW

    // Landing detection: track whether the player was airborne last frame.
  ...
  update({ input, solids, bounds, won }) {
    ...
    // -----------------------
    // LANDING DETECTION
    // Fires once on the frame the player touches ground after being airborne.
    // Triggers dust particles (physics visual) + land sound + screen shake (bonus).
    // -----------------------
    if (this._wasAirborne && grounded && !p.dead && !won) {
      this._onLand();
    }
  ...
  _onLand() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      dust.vel.x = random(-1.5, 1.5);
      dust.vel.y = random(-2.5, -0.5);
      dust.color = color(210, 190, 150, 200);
      dust.stroke = "none";
      dust.life = 18;
    }

    this.events?.emit("player:landed", {});
    window.triggerScreenShake?.(2, 4);
  }

  // -----------------------
  // DAMAGE — skipped entirely when god mode is active
  // -----------------------
  damageFromX(sourceX) {
    // NEW: god mode check — if enabled, ignore all incoming damage
    if (this.debugOverlay?.godMode) return false;
