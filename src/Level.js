...
export class Level {
  constructor(pkg, assets, opts = {}) {
    ...
    this.debugOverlay = opts.debugOverlay || null; // NEW: receive debugOverlay
  ...
  build() {
    ...
    // NEW: pass debugOverlay so god mode check works in damageFromX
  ...
  restart() {
    ...
    // NEW: reset gravity to normal when restarting (clears moon gravity)
    world.gravity.y = this.GRAVITY;

    // Also reset the debug toggle so the HUD reflects reality
    if (this.debugOverlay) {
      this.debugOverlay.moonGravity = false;
    }
